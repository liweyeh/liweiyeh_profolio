...
const PortfolioItems = ({ config }) => {
    ...
    const isInView = (offset, ref) => {
        if (ref) {
            ...
            const viewport = window.innerHeight - ref.getBoundingClientRect().height / offset;
            if (top <= viewport) {
                return true;
            }
        }
    ...
    const { title, status, teamSize, workType, content, demo, demolink, img } = config;

    return (
        <Grow in={isInView(3, contentRef.current)} timeout={{ enter: 1000, exit: 0 }}>
